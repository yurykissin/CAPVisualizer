...
$script:CapAuditFindingMeta = @{
    ...
    'combined-risk-conditions'     = @{ impact = 3; likelihood = 3; threat = 'One grant control is shared across two different risk signals, so the recommended per-signal responses (secure password change for user risk vs. MFA for sign-in risk) cannot both be enforced.'; remediation = 'Split into two policies: one conditioned on user risk (require secure password change) and one on sign-in risk (require MFA).'; references = @('CISA SCuBA MS.AAD.2.1', 'CISA SCuBA MS.AAD.2.3'); logic = 'Detected when a single policy configures both userRiskLevels and signInRiskLevels; Entra combines them with OR under one grant control, preventing distinct remediation per signal.' }
}
